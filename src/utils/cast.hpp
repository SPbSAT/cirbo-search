...
[[maybe_unused]]
inline std::string gateStateToString(GateState const state) noexcept
{
    return state == GateState::TRUE ? "TRUE" : state == GateState::FALSE ? "FALSE" : "UNDEFINED";
}

[[maybe_unused]]
inline std::string gateStateToSATAnswer(GateState const state) noexcept
{
...
#endif  // CIRBO_SEARCH_UTILS_CAST_HPP
