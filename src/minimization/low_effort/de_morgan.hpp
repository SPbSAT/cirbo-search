        ...
        std::unique_ptr<NameEncoder> encoder)
    {
        ...
        for (GateId gateId : gate_sorting)
        {
            ...
            if (indexes_of_not.at(gateId) != SIZE_MAX) // если перед гейтом, есть фиктивный NOT
            {
                ...
                {
                    // если хотябы один User гейта не использует его отрицание (т.е. User не использовал правило де Моргана), 
                    // то гейт оставляем не тронутым, чтобы не дублировать поддерево
                    ...
                    else
                    {
                        ...
                        gate_info.at(indexes_of_not.at(gateId)) = {
                            ...
                            get_new_operands_(*circuit, gate_info, *encoder, indexes_of_not, gateId, new_gate_name_prefix, count_branches)
                        };
                    }
                }
                else if (
                    circuit->getGateType(gateId) == GateType::NAND
                    || circuit->getGateType(gateId) == GateType::NOR
                    || circuit->getGateType(gateId) == GateType::NXOR)
                {
                    // заменяем NOT + NOR/NAND/NXOR на OR/AND/XOR
                    gate_info.at(indexes_of_not.at(gateId)) = {
                        static_cast<GateType>(static_cast<uint8_t>(circuit->getGateType(gateId)) - 1),
                        operands
                    };

                    // если хотябы один User гейта не использует его отрицание, то необходимо перевесить гейты, то есть измениь тип текущего гейта на NOT. 
                    // Все User'ы использующие правило де Моргана будут ссылаться на OR/AND/XOR, остальные на NOT(OR/AND/XOR)
                    if (count_branches[gateId] != circuit->getGateUsers(gateId).size()) 
                    {
                        rehang = true;
                    }
                ...
                // сохраняем гейт в нетронутом виде или изменяем его тип на NOT при необходимости для не переподвешенных ссылок 
                if (rehang)
                {
