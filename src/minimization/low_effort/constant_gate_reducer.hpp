#ifndef CIRBO_SEARCH_MINIMIZATION_CONSTANT_GATE_REDUCER_HPP
#define CIRBO_SEARCH_MINIMIZATION_CONSTANT_GATE_REDUCER_HPP
        ...
        std::unique_ptr<NameEncoder> encoder)
    {
        ...
        for (GateId gate_id : std::ranges::reverse_view(gate_sorting))
        {
                ...
                gate_type == GateType::CONST_FALSE)
            {
                ...
                if (!cirbo::utils::symmetricOperatorQ(gate_type))
                {
            ...
            {
                log::error("Unsupported gate state");
                std::abort();
            }
        }
