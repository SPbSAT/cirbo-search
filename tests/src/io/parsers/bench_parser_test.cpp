...
void testBasicParse(std::string const& str, [[maybe_unused]] var_map map, gate_names const& gates)
{
    ...
    for (auto const& g : gates)
    {
        cirbo::log::getGlobalLogger().info(
            "<PARSER TEST> Checking gate encoding: ", g, "==", parser.encoder.encodeGate(g));
