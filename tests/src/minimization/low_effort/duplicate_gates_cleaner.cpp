#include <catch2/catch_test_macros.hpp>
#include <memory>
#include <sstream>
#include <string>

#include "core/structures/dag.hpp"
#include "core/types.hpp"
#include "io/parsers/bench_to_circuit.hpp"
#include "minimization/composition.hpp"
#include "minimization/strategy.hpp"

using namespace cirbo;
using namespace cirbo::minimization;

TEST_CASE("DuplicateGatesCleaner SimpleTest", "[duplicate_gates]")
{
    std::string const dag =
        "INPUT(0)\n"
        "INPUT(1)\n"
        "OUTPUT(4)\n"
        "2 = AND(0, 1)\n"
        "3 = AND(0, 1)\n"
        "4 = AND(2, 3)\n";

    std::istringstream stream(dag);
    cirbo::io::parsers::BenchToCircuit<cirbo::DAG> parser;
    parser.parseStream(stream);

    std::unique_ptr<cirbo::DAG> csat_instance = parser.instantiate();
    cirbo::utils::NameEncoder encoder         = parser.getEncoder();

    auto [circuit, _] = Composition<DAG, DuplicateGatesCleaner<DAG> >().apply(*csat_instance, encoder);

    REQUIRE(circuit->getNumberOfGates() == 4);
    REQUIRE(circuit->getGateType(0) == GateType::INPUT);
    REQUIRE(circuit->getGateType(1) == GateType::INPUT);
    REQUIRE(circuit->getGateType(2) == GateType::AND);
    REQUIRE(circuit->getGateOperands(2) == GateIdContainer({0, 1}));
    REQUIRE(circuit->getGateType(3) == GateType::AND);
    REQUIRE(circuit->getGateOperands(3) == GateIdContainer({2, 2}));
    REQUIRE(circuit->getOutputGates() == GateIdContainer({3}));
}

TEST_CASE("DuplicateGatesCleaner NoSorted", "[duplicate_gates]")
{
    std::string const dag =
        "INPUT(0)\n"
        "INPUT(1)\n"
        "OUTPUT(4)\n"
        "2 = AND(0, 1)\n"
        "3 = AND(1, 0)\n"
        "4 = AND(2, 3)\n";

    std::istringstream stream(dag);
    cirbo::io::parsers::BenchToCircuit<cirbo::DAG> parser;
    parser.parseStream(stream);

    std::unique_ptr<cirbo::DAG> csat_instance = parser.instantiate();
    cirbo::utils::NameEncoder encoder         = parser.getEncoder();

    auto [circuit, _] = Composition<DAG, DuplicateGatesCleaner<DAG> >().apply(*csat_instance, encoder);

    REQUIRE(circuit->getNumberOfGates() == 4);
    REQUIRE(circuit->getGateType(0) == GateType::INPUT);
    REQUIRE(circuit->getGateType(1) == GateType::INPUT);
    REQUIRE(circuit->getGateType(2) == GateType::AND);
    REQUIRE(circuit->getGateOperands(2) == GateIdContainer({0, 1}));
    REQUIRE(circuit->getGateType(3) == GateType::AND);
    REQUIRE(circuit->getGateOperands(3) == GateIdContainer({2, 2}));
    REQUIRE(circuit->getOutputGates() == GateIdContainer({3}));
}

TEST_CASE("DuplicateGatesCleaner SeveralLevels", "[duplicate_gates]")
{
    std::string const dag =
        "INPUT(0)\n"
        "INPUT(1)\n"
        "INPUT(2)\n"
        "\n"
        "OUTPUT(11)\n"
        "\n"
        "3 = OR(0, 1)\n"
        "4 = AND(0, 1)\n"
        "5 = AND(1, 0)\n"
        "6 = NOT(2)\n"
        "7 = OR(3, 4)\n"
        "8 = OR(3, 5)\n"
        "9 = NOT(7)\n"
        "10 = OR(6, 8)\n"
        "11 = AND(9, 10)\n";

    std::istringstream stream(dag);
    cirbo::io::parsers::BenchToCircuit<cirbo::DAG> parser;
    parser.parseStream(stream);

    std::unique_ptr<cirbo::DAG> csat_instance = parser.instantiate();
    cirbo::utils::NameEncoder encoder         = parser.getEncoder();

    auto [circuit, _] = Composition<DAG, DuplicateGatesCleaner<DAG> >().apply(*csat_instance, encoder);

    REQUIRE(circuit->getNumberOfGates() == 10);
    REQUIRE(circuit->getGateType(0) == GateType::INPUT);
    REQUIRE(circuit->getGateType(1) == GateType::INPUT);
    REQUIRE(circuit->getGateType(2) == GateType::OR);
    REQUIRE(circuit->getGateOperands(2) == GateIdContainer({0, 1}));
    REQUIRE(circuit->getGateType(3) == GateType::AND);
    REQUIRE(circuit->getGateOperands(3) == GateIdContainer({0, 1}));
    REQUIRE(circuit->getGateType(4) == GateType::OR);
    REQUIRE(circuit->getGateOperands(4) == GateIdContainer({2, 3}));
    REQUIRE(circuit->getGateType(5) == GateType::NOT);
    REQUIRE(circuit->getGateOperands(5) == GateIdContainer({4}));
    REQUIRE(circuit->getGateType(6) == GateType::INPUT);
    REQUIRE(circuit->getGateType(7) == GateType::NOT);
    REQUIRE(circuit->getGateOperands(7) == GateIdContainer({6}));
    REQUIRE(circuit->getGateType(8) == GateType::OR);
    REQUIRE(circuit->getGateOperands(8) == GateIdContainer({4, 7}));
    REQUIRE(circuit->getGateType(9) == GateType::AND);
    REQUIRE(circuit->getGateOperands(9) == GateIdContainer({5, 8}));
}

TEST_CASE("DuplicateGatesCleaner SeveralOutput", "[duplicate_gates]")
{
    std::string const dag =
        "INPUT(0)\n"
        "INPUT(1)\n"
        "OUTPUT(2)\n"
        "OUTPUT(3)\n"
        "2 = AND(0, 1)\n"
        "3 = AND(1, 0)\n";

    std::istringstream stream(dag);
    cirbo::io::parsers::BenchToCircuit<cirbo::DAG> parser;
    parser.parseStream(stream);

    std::unique_ptr<cirbo::DAG> csat_instance = parser.instantiate();
    cirbo::utils::NameEncoder encoder         = parser.getEncoder();

    auto [circuit, _] = Composition<DAG, DuplicateGatesCleaner<DAG> >().apply(*csat_instance, encoder);

    REQUIRE(circuit->getNumberOfGates() == 3);
    REQUIRE(circuit->getGateType(0) == GateType::INPUT);
    REQUIRE(circuit->getGateType(1) == GateType::INPUT);
    REQUIRE(circuit->getGateType(2) == GateType::AND);
    REQUIRE(circuit->getGateOperands(2) == GateIdContainer({0, 1}));
    REQUIRE(circuit->getOutputGates() == GateIdContainer({2, 2}));
}

TEST_CASE("DuplicateGatesCleaner SeveralLevelsMUX", "[duplicate_gates]")
{
    std::string const dag =
        "INPUT(0)\n"
        "INPUT(1)\n"
        "INPUT(2)\n"
        "\n"
        "OUTPUT(11)\n"
        "\n"
        "3 = MUX(0, 1, 12)\n"
        "4 = MUX(0, 12, 1)\n"
        "5 = MUX(0, 12, 1)\n"
        "6 = NOT(2)\n"
        "7 = OR(3, 4)\n"
        "8 = OR(5, 3)\n"
        "9 = NOT(7)\n"
        "10 = OR(6, 8)\n"
        "11 = AND(9, 10)\n"
        "12 = CONST(0)";

    std::istringstream stream(dag);
    cirbo::io::parsers::BenchToCircuit<cirbo::DAG> parser;
    parser.parseStream(stream);

    std::unique_ptr<cirbo::DAG> csat_instance = parser.instantiate();
    cirbo::utils::NameEncoder encoder         = parser.getEncoder();

    auto [circuit, _] = Composition<DAG, DuplicateGatesCleaner<DAG> >().apply(*csat_instance, encoder);

    REQUIRE(circuit->getNumberOfGates() == 11);
    REQUIRE(circuit->getGateType(0) == GateType::INPUT);
    REQUIRE(circuit->getGateType(1) == GateType::INPUT);
    REQUIRE(circuit->getGateType(2) == GateType::CONST_FALSE);
    REQUIRE(circuit->getGateOperands(2) == GateIdContainer({}));
    REQUIRE(circuit->getGateType(3) == GateType::MUX);
    REQUIRE(circuit->getGateOperands(3) == GateIdContainer({0, 1, 2}));
    REQUIRE(circuit->getGateType(4) == GateType::MUX);
    REQUIRE(circuit->getGateOperands(4) == GateIdContainer({0, 2, 1}));
    REQUIRE(circuit->getGateType(5) == GateType::OR);
    REQUIRE(circuit->getGateOperands(5) == GateIdContainer({3, 4}));
    REQUIRE(circuit->getGateType(6) == GateType::NOT);
    REQUIRE(circuit->getGateOperands(6) == GateIdContainer({5}));
    REQUIRE(circuit->getGateType(7) == GateType::INPUT);
    REQUIRE(circuit->getGateType(8) == GateType::NOT);
    REQUIRE(circuit->getGateOperands(8) == GateIdContainer({7}));
    REQUIRE(circuit->getGateType(9) == GateType::OR);
    REQUIRE(circuit->getGateOperands(9) == GateIdContainer({5, 8}));
    REQUIRE(circuit->getGateType(10) == GateType::AND);
    REQUIRE(circuit->getGateOperands(10) == GateIdContainer({6, 9}));
}

TEST_CASE("DuplicateGatesCleaner ReducibleDuplicateOfOperands", "[duplicate_gates]")
{
    std::string const dag =
        "INPUT(0)\n"
        "INPUT(1)\n"
        "\n"
        "2 = AND(1, 0)\n"
        "3 = AND(0, 1)\n"
        "4 = AND(0, 2, 3)\n"
        "5 = AND(0, 2)\n"
        "6 = OR(4, 5)\n"
        "\n"
        "OUTPUT(6)\n";

    std::istringstream stream(dag);
    cirbo::io::parsers::BenchToCircuit<cirbo::DAG> parser;
    parser.parseStream(stream);

    std::unique_ptr<cirbo::DAG> csat_instance = parser.instantiate();
    cirbo::utils::NameEncoder encoder         = parser.getEncoder();

    auto [circuit, _] = Composition<DAG, DuplicateGatesCleaner<DAG> >().apply(*csat_instance, encoder);

    REQUIRE(circuit->getNumberOfGates() == 5);
    REQUIRE(circuit->getGateType(0) == GateType::INPUT);
    REQUIRE(circuit->getGateType(1) == GateType::INPUT);
    REQUIRE(circuit->getGateType(2) == GateType::AND);
    REQUIRE(circuit->getGateOperands(2) == GateIdContainer({0, 1}));
    REQUIRE(circuit->getGateType(3) == GateType::AND);
    REQUIRE(circuit->getGateOperands(3) == GateIdContainer({0, 2, 2}));
    REQUIRE(circuit->getGateType(4) == GateType::OR);
    REQUIRE(circuit->getGateOperands(4) == GateIdContainer({3, 3}));
}

TEST_CASE("DuplicateGatesCleaner XorNotReducible", "[duplicate_gates]")
{
    std::string const dag =
        "INPUT(0)\n"
        "INPUT(1)\n"
        "\n"
        "2 = AND(1, 0)\n"
        "3 = AND(0, 1)\n"
        "4 = XOR(0, 2, 3)\n"
        "5 = XOR(0, 2)\n"
        "6 = OR(4, 5)\n"
        "\n"
        "OUTPUT(6)\n";

    std::istringstream stream(dag);
    cirbo::io::parsers::BenchToCircuit<cirbo::DAG> parser;
    parser.parseStream(stream);

    std::unique_ptr<cirbo::DAG> csat_instance = parser.instantiate();
    cirbo::utils::NameEncoder encoder         = parser.getEncoder();

    auto [circuit, _] = Composition<DAG, DuplicateGatesCleaner<DAG> >().apply(*csat_instance, encoder);

    REQUIRE(circuit->getNumberOfGates() == 6);
    REQUIRE(circuit->getGateType(0) == GateType::INPUT);
    REQUIRE(circuit->getGateType(1) == GateType::INPUT);
    REQUIRE(circuit->getGateType(2) == GateType::AND);
    REQUIRE(circuit->getGateOperands(2) == GateIdContainer({0, 1}));
    REQUIRE(circuit->getGateType(3) == GateType::XOR);
    REQUIRE(circuit->getGateOperands(3) == GateIdContainer({0, 2, 2}));
    REQUIRE(circuit->getGateType(4) == GateType::XOR);
    REQUIRE(circuit->getGateOperands(4) == GateIdContainer({0, 2}));
    REQUIRE(circuit->getGateType(5) == GateType::OR);
    REQUIRE(circuit->getGateOperands(5) == GateIdContainer({3, 4}));
}
